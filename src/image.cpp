...
bool Image::Load(const char* filename)
{
    ...
    if (filename[len-4] == '.' && filename[len-3] == 't' &&
        filename[len-2] == 'g' && filename[len-1] == 'a')
    {
        TGA_Info* info = TGA_Load(filename);
        ASSERT(info);
        ...
        // copy image data
        memcpy(m_mips[0].data, info->imageData, width * height * pixelSize);
    ...
    else if (filename[len-4] == '.' && filename[len-3] == 'p' &&
    ...
    fprintf(stderr, "Error: Unsupported image type \"%s\"\n", filename);
    return false;
}
...
static void rgb_lum(const unsigned char* src, unsigned char* dst, int width, int height)
{
    ...
    for (int i = 0; i < height * width; i++, s += kSrcPixelSize, d += kDstPixelSize)
    {
        ...
        unsigned char g = *(s + 1);
        unsigned char b = *(s + 2);
        float lum = 0.2126f * r + 0.7152f * g + 0.0722 * b;
        *d = (unsigned char)lum;
    }
...
static float s_gaussianKernel[] = {0.044919223845156, 0.12210310992677, 0.044919223845156,
                                   0.12210310992677,  0.33191066491228, 0.12210310992677,
                                   0.044919223845156, 0.12210310992677, 0.044919223845156};

static float s_wideGaussianKernel[] = {0.00010678874539336, 0.0021449092885793, 0.005830467942838, 0.0021449092885793, 0.00010678874539336,
                                       0.0021449092885793, 0.043081654712647, 0.11710807914534, 0.043081654712647, 0.0021449092885793,
                                       0.005830467942838, 0.11710807914534, 0.31833276350651, 0.11710807914534, 0.005830467942838,
                                       0.0021449092885793, 0.043081654712647, 0.11710807914534, 0.043081654712647, 0.0021449092885793,
                                       0.00010678874539336, 0.0021449092885793, 0.005830467942838, 0.0021449092885793, 0.00010678874539336};
